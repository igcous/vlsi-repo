...
class AmbaClass(object):
	def __init__(self,
		outfile = 'C:/Users/AK124602/Documents/Vivado/2021.2/mp2_6/scripts/python/in_instr.mem'
		#outfile = './vhdl/in.mem'
		):
		self.addr_width = 32
		self.data_width = 32
	...
	def burst_write(self,fp,start_address,num_address,size):
		if size == 'byte':
			max_data = 2**8-1
			addr_mult = 1
		elif size == 'half word':
	...
	def burst_read(self,fp,start_address,num_address,size):
		if size == 'byte':
			addr_mult = 1
		elif size == 'half word':
			addr_mult = 2
...
def main():
	...
	with open(amba_1.outfile, 'w') as outfile:
		amba_1.burst_write(outfile,0,2,'word')
		amba_1.burst_read(outfile,0,2,'word')
